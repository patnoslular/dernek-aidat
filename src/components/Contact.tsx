...
export default function Contact({ members, duesRules }: ContactProps) {
  ...
  const currentMonth = new Date().getMonth(); // 0-11 (Ocak-Aralık)
  ...
  const calculateDebt = (member: Member) => {
    ...
    // Yıl başından (0) mevcut aya kadar olan borcu hesapla
...
Derneğimizin faaliyetlerini sürdürebilmesi ve sizlere daha iyi hizmet sunabilmemiz için aidat ödemeleri büyük önem taşımaktadır.

Ödemelerinizi Dernek Saymanımız M. Maşallah Utkan adına kayıtlı:
T.C. Ziraat Bankası
IBAN: TR19 0001 0007 1162 5394 0550 03

numaralı hesaba yatırmanızı rica ederiz. Teşekkür ederiz.`;
  ...
  const handleBulkSend = async () => {
    ...
    const confirm = window.confirm(`${debtorMembers.length} üyeye hatırlatma mesajı penceresi açılacak. Onaylıyor musunuz?`);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-white tracking-tight">İletişim & Borç Takibi</h2>
          <p className="text-slate-500 text-sm">Borçlu üyeleri listeleyin ve WhatsApp üzerinden hızlı hatırlatma gönderin.</p>
        </div>
        <button 
          onClick={handleBulkSend}
          disabled={debtorMembers.length === 0}
          className="bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 disabled:cursor-not-allowed text-white px-5 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center justify-center gap-2 shadow-lg shadow-emerald-500/20"
        >
          <MessageSquare className="w-4 h-4" />
          <span>Toplu Hatırlat ({debtorMembers.length})</span>
        </button>
      </div>
      ...
      <div className="glass-card p-4">
        <div className="relative">
          ...
          <input 
            ...
            placeholder="İsim veya telefon ile borçlu ara..." 
            className="bg-slate-900/50 border border-slate-800 rounded-xl py-2.5 pl-10 pr-4 text-sm text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 w-full transition-all"
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {debtorMembers.map((member) => (
          <motion.div 
            ...
            key={member.id}
            className="glass-card p-5 flex flex-col gap-4 border-l-4 border-rose-500 group hover:border-rose-400 transition-all"
          >
            <div className="flex items-start justify-between">
              ...
              <div className="text-right">
                <p className="text-[10px] text-slate-500 font-bold uppercase">Borç</p>
                <p className="text-lg font-bold text-rose-400">₺{member.debt.toLocaleString()}</p>
              </div>
            </div>

            <div className="flex items-center gap-2 text-xs text-slate-400 bg-slate-900/50 p-2.5 rounded-lg border border-slate-800/50">
            ...
            <div className="flex items-center gap-2 mt-2">
              <a 
                ...
                target="_blank"
                rel="noreferrer"
                className="flex-1 bg-emerald-600/10 hover:bg-emerald-600 text-emerald-500 hover:text-white border border-emerald-500/20 px-3 py-2.5 rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-2"
              >
              ...
              <a 
                ...
                className="p-2.5 rounded-lg bg-slate-800 text-slate-400 hover:text-white hover:bg-slate-700 transition-colors border border-slate-700"
              >
          ...
          <div className="col-span-full py-20 flex flex-col items-center justify-center text-center space-y-4">
            ...
            <div>
              ...
              <p className="text-slate-500 text-sm max-w-xs mx-auto mt-1">
                Harika! Tüm üyelerimiz aidat ödemelerini gerçekleştirmiş görünüyor.
              </p>
            </div>
          </div>
        )}
